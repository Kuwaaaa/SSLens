...
这份文档解释 lumen 是什么、为什么存在。它不是当前实现 checklist。当前状态看 `docs/project-status.md`。AI 助手入口和阅读路由看 `AGENTS.md`。
...
反过来就是 0.x 的陷阱：先建知识基础设施，再希望人们后来抵达，最后发现他们不会来。
...
Atlas 是真实方向，但不属于 lumen 范围。lumen 应保留足够上下文，让部分 Lens 未来可以成为 node candidate；但不能把当前 beta UI 变成知识管理工具。
